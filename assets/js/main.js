...
(function () {
  ...
  /* ── Card tilt ──────────────────────────────────────────
     Leans the "What I do" cards toward the pointer and moves a glare with
     it. Everything here is decoration: without this file the cards are
     still glass, still lift, still take the accent — only the lean is
     missing.

     Targets every direct child of [data-tilt] rather than a class, so the
     behaviour can be pointed at another set without renaming anything.
  ...
  if (menuItems.length && nav) {
    ...
    // Must match the CSS drawer breakpoint exactly (max-width: 900px).
